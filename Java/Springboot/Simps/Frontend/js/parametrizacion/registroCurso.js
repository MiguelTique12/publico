...
$(document).ready(function () {
    var table = $('#datatable').DataTable({
        ...
        ajax: {
            url: 'http://localhost:9000/simps/api/parametrizacion/curses/datatable?page=0&size=150&column_order=name&column_direction=grade/',
...
function Add() {
    ...
    var url = actualizar ? 'http://localhost:9000/simps/api/parametrizacion/curses/' + id : 'http://localhost:9000/simps/api/parametrizacion/curses';
...
function deleteById(id) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'http://localhost:9000/simps/api/parametrizacion/curses/' + id,
                method: "delete",
                headers: {
                    "Content-Type": "application/json"
                }
...
function findById(id) {
    $.ajax({
        ...
        $("#id").val(item.id)
        $("#name").val(item.name)
        $("#gradeId").val(item.gradeId.id)
        $("#techniqueId").val(item.techniqueId.id)
        $("#classroomId").val(item.classroomsId.id)
        if (item.state == true) {
