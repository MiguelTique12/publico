...
$(document).ready(function () {
    var table = $('#datatable').DataTable({
        ...
        ajax: {
            url: 'http://localhost:9000/simps/api/parametrizacion/schendulesSubjects/datatable?page=0&size=150&column_order=curse_id&column_direction=subject/',
        ...
        columns: [
            ...
            { "data": "subjectId" },
            {
                ...
                "render": function (data, type, row) {
                    var combinedTime = row.startTime + " - " + row.endTime;
                    return combinedTime;
                }
            },
            {
...
function Add() {
    ...
    var url = actualizar ? 'http://localhost:9000/simps/api/parametrizacion/schendulesSubjects/' + id : 'http://localhost:9000/simps/api/parametrizacion/schendulesSubjects';
...
function deleteById(id) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'http://localhost:9000/simps/api/parametrizacion/schendulesSubjects/' + id,
                method: "delete",
                headers: {
                    "Content-Type": "application/json"
                }
...
function findById(id) {
    $.ajax({
        ...
        $("#id").val(item.id)
        $("#subjectId").val(item.subjectId.id)
        $("#schenduleId").val(item.schenduleId.id)
        $("#curseId").val(item.curseId.id)
        if (item.state == true) {
