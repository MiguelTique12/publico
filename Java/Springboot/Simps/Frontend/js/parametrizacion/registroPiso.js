...
$(document).ready(function () {
  var table = $('#datatable').DataTable({
    ...
    ajax: {
      url: 'http://localhost:9000/simps/api/parametrizacion/floors/datatable?page=0&size=150&column_order=floorNumber&column_direction=asc',
...
function Add() {
  ...
  var url = actualizar ? 'http://localhost:9000/simps/api/parametrizacion/floors/' + id : 'http://localhost:9000/simps/api/parametrizacion/floors';


  var floorNumberText = $('#floorNumber').val();
  var floorNumber = parseInt(floorNumberText);

  if (!isNaN(floorNumber)) {
    // El valor de floorNumber es un número válido
    var data = {
    ...
    // El valor de floorNumber no es un número válido, puedes manejar el error aquí
    console.error('El valor de floorNumber no es un número válido');
  };
  ...
  $.ajax({
    ...
    var mensaje = actualizar ? 'Hecho! Persona Actualizada' : 'Hecho! Persona Guardada';
...
function deleteById(id) {
  ...
  swalWithBootstrapButtons.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: 'http://localhost:9000/simps/api/parametrizacion/floors/' + id,
        method: "delete",
        headers: {
          "Content-Type": "application/json"
        }
...
function findById(id) {
  $.ajax({
    ...
    $("#id").val(item.id)
    $("#floorNumber").val(item.floorNumber)
    $("#headquaterId").val(item.headquaterId.id)
    if (item.state == true) {
