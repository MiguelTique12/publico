...
$(document).ready(function () {
    $('#datatable').DataTable({
        ajax: {
            url: 'http://localhost:9000/simps/api/parametrizacion/subjects/datatable?page=0&size=150&column_order=name&column_direction=asc',
...
function Add() {
    ...
    var url = actualizar ? 'http://localhost:9000/simps/api/parametrizacion/subjects/' + id : 'http://localhost:9000/simps/api/parametrizacion/subjects';
    ...
    $.ajax({
        ...
        var mensaje = actualizar ? 'Hecho! elemento creado' : 'Hecho! el econtrado indefinido';
...
function deleteById(id) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'http://localhost:9000/simps/api/parametrizacion/subjects/' + id,
                method: "delete",
                headers: {
                    "Content-Type": "application/json"
                }
...
function findById(id) {
    $.ajax({
        ...
        $("#code").val(item.code)
        $("#name").val(item.name)
        $("#teacherId").val(item.teacherId.id)
        if (item.state == true) {
