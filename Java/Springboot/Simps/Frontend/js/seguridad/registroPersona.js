...
$(document).ready(function () {
    $('#datatable').DataTable({
        ajax: {
            url: 'http://localhost:9000/simps/api/seguridad/persons/datatable?page=0&size=150&column_order=secondName&column_direction=asc',
        ...
        columns: [
            ...
            // Columna de Nombres con primer nombre y segundo nombre
            {
                "data": function (row, type, set, meta) {
                    return row.firstName + (row.secondName ? ' ' + row.secondName : '');
                },
                "name": "Nombres",
                "orderable": false,
                "searchable": true,
            },
            // Columna de Apellidos con primer apellido y segundo apellido
            {
                "data": function (row, type, set, meta) {
                    return row.firstLastName + (row.secondLastName ? ' ' + row.secondLastName : '');
                },
                "name": "Apellidos",
                "orderable": false,
                "searchable": true,
            ...
            { "data": "age" },
            {
                ...
                render: function (data, type, row) {
                    return data ? "Masculino" : "Femenino";
                }
            },
...
function Add() {
    ...
    var url = actualizar ? 'http://localhost:9000/simps/api/seguridad/persons/' + id : 'http://localhost:9000/simps/api/seguridad/persons';
    ...
    $.ajax({
        ...
        var mensaje = actualizar ? 'Hecho! persona actualizada' : 'Hecho! persona creada';
        ...
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: (jqXHR.responseJSON.message == "Could not commit JPA transaction") ? 'Hay un error en los datos' : jqXHR.responseJSON.message,
...
function deleteById(id) {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'http://localhost:9000/simps/api/seguridad/persons/' + id,
                method: "delete",
                headers: {
                    "Content-Type": "application/json"
                }
